{% if  var('klaviyo_flow_action_tracking_utm_param') %}
{{ config( enabled = True ) }}
{% else %}
...
table_pattern=var('klaviyo_flow_action_tracking_utm_param_table_pattern'),
database=var('raw_database')) %}

{% for i in relations %}
        select
        coalesce(id, 'NA') as flow_action_id,
        {{extract_nested_value("utm_params","name","string")}} as name,
        {{extract_nested_value("utm_params","value","string")}} as value,
