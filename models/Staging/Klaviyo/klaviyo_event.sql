-- depends_on: {{ ref('klaviyo_campaign') }}
{% if  var('klaviyo_event') and var('klaviyo_campaign') %}
{{ config( enabled = True ) }}
{% else %}
...
table_pattern=var('klaviyo_event_table_pattern'),
...
{% for i in relations %}
        ...
        left join {{ref('klaviyo_campaign')}} c on event_properties.CampaignName = c.name
    {% if is_incremental() %}
